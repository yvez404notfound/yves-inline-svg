...
- `test/InlineSVG.test.tsx` contains executable component coverage using `node:test`, Testing Library, JSDOM, and `react-dom/server`; `test/sanitize.test.ts` contains sanitizer bypass and preservation coverage.
...
6. Add or update behavior tests in `test/InlineSVG.test.tsx` and sanitizer policy tests in `test/sanitize.test.ts` as appropriate. Prefer observable rendering, accessibility role queries, callback, sanitization, fetch, and server-rendering assertions over source-text checks.
